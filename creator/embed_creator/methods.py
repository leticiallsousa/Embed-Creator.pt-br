...
class CreatorMethods:
    ...
    async def edit_footer(self, interaction: Interaction) -> None:
        ...
        modal.add_item(
            TextInput(
                ...
                placeholder="Text para ser mostrado no rodapé da embed",
            )
        )
        modal.add_item(
            TextInput(
    ...
    async def add_field(self, interaction: Interaction) -> None:
        ...
        modal.add_item(
            TextInput(label="Valor do campo", max_length=2000, style=TextStyle.paragraph)
        )
        modal.add_item(
            TextInput(
                ...
                default="True",
                max_length=5,
                placeholder="Os campos ficarão na mesma linha? Sim (True) ou Não (False)",
            )
        )
        await interaction.response.send_modal(modal)
        await modal.wait()
